...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s08a.html");
const canonicalSlug="2027-suteuk-hwajak-s08a";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("국제 자원봉사단 선발 면접")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("국제 자원봉사단 선발 면접");
  });
...
console.log("VERIFY_8A",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("8A live verification failed");
console.log("REPAIR_8A_OK");
