...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s06a.html");
const canonicalSlug="2027-suteuk-hwajak-s06a";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("나눔의 날 행사 준비")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("나눔의 날 행사 준비");
  });
...
console.log("VERIFY_6A",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("6A live verification failed");
console.log("REPAIR_6A_OK");
