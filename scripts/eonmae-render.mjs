...
export function renderPages(pages,contentDir,assetsDir){
  ...
  for(const page of pages){
    ...
    const metadata=[`<!-- title: [2027 수능특강 언어와 매체] ${page.title} 해설 및 변형문제 -->`,`<!-- slug: ${page.slug} -->`,`<!-- status: publish -->`,`<!-- type: ${type} -->`,categories?`<!-- categories: ${categories} -->`:'',`<!-- revision: ${Number.isFinite(revision)?revision+1:2} -->`,postId?`<!-- post_id: ${postId} -->`:'',`<!-- excerpt: 2027 수능특강 언어와 매체 ${page.crumb} ${page.title}의 상세 개념 설명, 클릭형 핵심어 15개, 분석 흐름 4개, 출제 포인트 10개, 변형문제 10제와 50개 선택지별 해설입니다. -->`].filter(Boolean).join('\n');
...
export function renderIndex(pages,contentDir){
  ...
  const metadata=['<!-- title: 2027 수능특강 언어와 매체 전체 해설 및 변형문제 -->','<!-- slug: 2027-수능특강-언어와-매체-전체-해설-및-변형-문제 -->','<!-- status: publish -->','<!-- type: page -->',`<!-- revision: ${Number.isFinite(revision)?revision+1:2} -->`,postId?`<!-- post_id: ${postId} -->`:'','<!-- excerpt: 2027 수능특강 언어와 매체 공식 PDF 기준 42개 단원의 상세 개념 설명과 420개 변형문제 통합 목록입니다. -->'].filter(Boolean).join('\n');
  ...
  fs.writeFileSync(oldFile,`${metadata}\n<style>${css}</style><main class="eix"><header class="hero"><h1>2027 수능특강 언어와 매체 전체 통합 목록</h1><p>공식 PDF 기준 개념 학습 7개, 언어 15개, 매체 12개, 통합 6개, 실전 2개 등 총 42개 자료를 전면 재작성했습니다. 각 글에는 단원 고유의 상세 설명, 클릭형 핵심어 15개, 분석 흐름 빈칸 4개, 정확한 개념 정의, 출제 포인트 10개, 유형을 달리한 변형문제 10제와 선택지별 해설이 수록되어 있습니다.</p></header><p class="note">단원명·문법 개념·매체 제재를 입력하면 해당 자료만 볼 수 있습니다.</p><input id="es" class="search" type="search" placeholder="예: 최소 대립쌍, 겹받침, 카드 뉴스, 아쿠아포닉스" aria-label="언어와 매체 자료 검색">${sections}<p id="none" class="none">검색 결과가 없습니다.</p></main><script>${js}</script>\n`,'utf8');
}
