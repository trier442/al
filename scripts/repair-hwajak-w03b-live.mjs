...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-w03b.html");
const canonicalSlug="2027-suteuk-hwajak-w03b";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("영상 제작 동아리 자기소개서")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("영상 제작 동아리 자기소개서");
  });
...
console.log("VERIFY_W03B",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("W03B live verification failed");
console.log("REPAIR_W03B_OK");
