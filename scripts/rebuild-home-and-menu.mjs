...
const common1=[
 ["비상 박영민",base+"/비상-박영민-공통국어1/"],
 ["비상 강호영",base+"/비상-강호영-공통국어1/"],
 ["천재 김수학",base+"/천재-김수학-공통국어1/"],
 ["천재 김종철",base+"/천재-김종철-공통국어1/"],
 ["미래엔 신유식",base+"/미래엔-신유식-공통국어1/"],
 ["지학사 김철회",base+"/지학사-김철회-공통국어1/"],
 ["동아",base+"/동아-공통국어1/"],
 ["창비",base+"/창비-공통국어1/"],
 ["해냄에듀",base+"/해냄에듀-공통국어1/"]
];
const reading2027=base+"/2027-%EC%88%98%EB%8A%A5%ED%8A%B9%EA%B0%95-%EB%8F%85%EC%84%9C-%EC%A0%84%EC%B2%B4-%EC%A7%80%EB%AC%B8-%ED%95%B4%EC%84%A4-%EB%B0%8F-%EB%B3%80%ED%98%95-%EB%AC%B8%EC%A0%9C/";
...
<section id="suteuk2027" class="section"><h2>고3 / N수 · 2027 수능특강</h2><p>영역별 자료를 바로 확인하세요.</p><div class="grid" style="grid-template-columns:repeat(3,minmax(0,1fr))"><a class="card big" href="${P.lit.link}"><strong>2027 수능특강 문학</strong><span>문학 카테고리 ${litCount}개 자료 · 전체 목차 · 상세 해설 · 변형문제 →</span></a><a class="card big" href="${reading2027}"><strong>2027 수능특강 독서</strong><span>전체 72지문 · 요약 · 상세 해설 · 변형문제 →</span></a><a class="card big" href="${P.hwajak.link}"><strong>2027 수능특강 화법과 작문</strong><span>화법·작문 통합 목차 · 상세 해설 · 변형문제 →</span></a></div><p style="margin-top:16px"><a href="${suteukHub.link}" style="font-weight:800;color:#17608e">2027 수능특강 자료 전체 보기 →</a></p></section>
...
let menu=Array.isArray(menus)&&(
  menus.find(x=>x.slug==="modu-main-nav") ||
  menus.find(x=>x.name==="모두의 국어 메인 내비게이션") ||
  menus.find(x=>x.slug==="modu-reviewed-main")
);
if(!menu) menu=await wp("/wp/v2/menus",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:"모두의 국어 메인 메뉴 2026",slug:"modu-main-2026"})});
...
await add("홈",base+"/");
const g1=await add("고1 국어",base+"/#common1");
const c1=await add("공통국어1",base+"/#common1",g1);
for(const [name,url] of common1) await add(name,url,c1);
...
for(const needed of ["홈","고1 국어","공통국어1","공통국어2","2027 수능특강","문학","독서","화법과 작문","자료 검색"]) if(!titles.includes(needed)) throw new Error("MENU_MISSING "+needed);
...
for(const marker of ["공통국어1","공통국어2","2027 수능특강 문학","2027 수능특강 독서","비상 박영민","미래엔 신유식"]) if(!publicHome.includes(marker)) throw new Error("PUBLIC_HOME_MISSING "+marker);
