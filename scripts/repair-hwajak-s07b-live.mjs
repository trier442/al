...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s07b.html");
const canonicalSlug="2027-suteuk-hwajak-s07b";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("배려하는 말하기와 나-전달법")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("배려하는 말하기와 나-전달법");
  });
...
console.log("VERIFY_7B",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("7B live verification failed");
console.log("REPAIR_7B_OK");
