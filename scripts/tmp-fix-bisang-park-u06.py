...
# 선택지 길이만으로 정답을 짐작하기 어렵도록 오답 표현을 자연스럽게 조정
s=s.replace('<div class="choice">① ㉠, ㉡</div>','<div class="choice">① ㉠, ㉡, ㉢, ㉤</div>',1)
s=s.replace('<div class="choice">⑤ ‘되’와 ‘돼’는 발음이 비슷하므로 표기에서 구별할 필요가 없다.</div>','<div class="choice">⑤ ‘되’와 ‘돼’는 발음이 비슷하므로 활용 형태나 문장 속 기능을 살피지 않고 같은 표기로 바꾸어 써도 된다.</div>',1)
s=s.replace('<div class="choice">① 몇일 / 나예요 / 곰곰이</div>','<div class="choice">① 몇일 / 나예요 / 곰곰이 / 갔대</div>',1)
s=s.replace('<div class="choice">① ‘안’과 ‘않-’은 모두 용언의 어간이므로 뒤의 말과 붙여 쓴다.</div>','<div class="choice">① ‘안’과 ‘않-’은 모두 용언의 어간이므로 뒤의 말과 반드시 붙여 써야 하며 서로 바꾸어 써도 된다.</div>',1)
s=s.replace('<div class="choice">⑤ 나의 ‘만큼’과 다의 ‘대로’는 모두 단위를 나타내는 명사이다.</div>','<div class="choice">⑤ 나의 ‘만큼’과 다의 ‘대로’는 모두 수량과 단위를 나타내는 자립 명사이므로 언제나 앞말과 띄어 쓴다.</div>',1)
...
for i,a in enumerate(arts,1):
    ...
    print('Q',i,'lens',lens,'answer',ans+1)
    if not lens[ans] < max(lens[:ans]+lens[ans+1:]): issues.append((i,lens))
print('LENGTH_ISSUES',issues)
