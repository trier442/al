...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s08b.html");
const canonicalSlug="2027-suteuk-hwajak-s08b";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("인간 뇌 오가노이드 연구 토론")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("인간 뇌 오가노이드 연구 토론");
  });
...
console.log("VERIFY_8B",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("8B live verification failed");
console.log("REPAIR_8B_OK");
