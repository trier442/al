...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-w05a.html");
const canonicalSlug="2027-suteuk-hwajak-w05a";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("지역 축제 개선 건의문")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("지역 축제 개선 건의문");
  });
...
console.log("VERIFY_W05A",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("W05A live verification failed");
console.log("REPAIR_W05A_OK");
