...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s07a.html");
const canonicalSlug="2027-suteuk-hwajak-s07a";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("전국 체육 대회 공동 개최 협상")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("전국 체육 대회 공동 개최 협상");
  });
...
console.log("VERIFY_7A",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("7A live verification failed");
console.log("REPAIR_7A_OK");
