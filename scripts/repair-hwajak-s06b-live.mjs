...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-s06b.html");
const canonicalSlug="2027-suteuk-hwajak-s06b";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("친구의 부탁을 효과적으로 거절하기")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("친구의 부탁을 효과적으로 거절하기");
  });
...
console.log("VERIFY_6B",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("6B live verification failed");
console.log("REPAIR_6B_OK");
