...
const targetFile=path.join("wordpress-content","2027-suteuk-hwajak-w02a.html");
const canonicalSlug="2027-suteuk-hwajak-w02a";
...
try{
  ...
  for(const status of ["publish","draft","pending","private","future"]){
    try{
      const rows=await wp(base+`/wp-json/wp/v2/posts?context=edit&status=${status}&per_page=100&orderby=id&order=desc&search=${encodeURIComponent("관광세 도입 논설문")}`);
  ...
  const candidates=all.filter(x=>{
    ...
    return slug.startsWith(canonicalSlug) || title.includes("관광세 도입 논설문");
  });
...
console.log("VERIFY_W02A",JSON.stringify({id:verified.id,status:verified.status,slug:verified.slug,link:verified.link,front:f}));
if(verified.status!=="publish" || !(f.status>=200&&f.status<400)) throw new Error("W02A live verification failed");
console.log("REPAIR_W02A_OK");
